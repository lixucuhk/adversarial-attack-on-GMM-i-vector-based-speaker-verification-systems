...
if [ $stage -le 1 ]; then
  echo 'stage 1, extract LPMS features, and generate VAD files'
  # Make LPMS features and compute the energy-based VAD for each dataset
  python3 extract_lps.py --train-wav-file data/train/wav.scp --test-wav-file data/voxceleb1_test/wav.scp --num-workers 5 \
                         --out-dir data/LPMS --train-rstfilename train --test-rstfilename test || exit 1

  cp data/LPMS/train.scp data/train/feats.scp || exit 1
  cp data/LPMS/test.scp data/voxceleb1_test/feats.scp || exit 1

  for name in train voxceleb1_test; do
    python3 local/compute_vad.py --feats-scp data/${name}/feats.scp --vad-ark data/LPMS/${name}_vad.ark --vad-scp data/${name}/vad.scp || exit 1
    python3 local/utt2num_frame.py --vad-scp data/${name}/vad.scp --utt2num_frames data/${name}/utt2num_frames || exit 1

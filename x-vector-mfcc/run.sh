...
musan_root=/scratch/xli/Data_Source/musan
...
if [ $stage -le 0 ]; then
  echo 'stage 0, prepare data-related files and acoustic features (only copy scp files from MFCC-ivec to save memory).'
  mkdir -p data
  cp -r ../i-vector-mfcc/data/train data/train || exit 1
  cp -r ../i-vector-mfcc/data/voxceleb1_test data/voxceleb1_test || exit 1
  utils/fix_data_dir.sh data/train
  utils/fix_data_dir.sh data/voxceleb1_test
fi

# In this section, we augment the VoxCeleb1 data with reverberation,
# noise, music, and babble, and combine it with the clean data.
if [ $stage -le 1 ]; then
  echo 'stage 1, augment the Voxceleb1 data.'
  frame_shift=0.01
...
if [ $stage -le 2 ]; then
  echo 'stage 2, make mfccs for the augmented data.'
  # Take a random subset of the augmentations
  utils/subset_data_dir.sh data/train_aug 150000 data/train_aug_1m
...
local/nnet3/xvector/run_xvector.sh --train-stage -1 \
